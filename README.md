...
This orchestrator provides a **unified backend API** that processes candidates through a complete HR workflow in one sequential operation:
...
│   │   │   ├── FormSection.tsx        ← Job role and questions form
│   │   │   ├── ResultsViewer.tsx      ← Results display
│   │   │   ├── LoadingSpinner.tsx
│   │   │   └── ErrorMessage.tsx
│   │   ├── App.tsx                    ← Page state and workflow orchestration
│   │   ├── main.tsx
│   │   ├── index.css
│   │   ├── types.ts
│   │   └── api.ts                     ← Calls the backend `/orchestrate` endpoint
│   ├── index.html
│   ├── package.json
│   ├── vite.config.ts
│   ├── tailwind.config.js
...
│   ├── .env
│   ├── requirements.txt
│   ├── read_pdfs/                      ← Stored resume text extracts
│   │   ├── resume_text_1.txt
│   │   ├── resume_text_2.txt
│   │   └── resume_text_3.txt
│   └── src/
...
│       ├── onboarding_agent.py
│       ├── orchestrator.py            ← FastAPI entry point on port 9000
│       ├── policy_agent.py
│       ├── policy_documents.json      ← legacy copy of policy data
│       └── resume_agent.py
├── .github/
├── README.md
└── .gitignore
...
**`backend/src/resume_agent.py`:**
```python
GEMINI_API_KEY = "YOUR_RESUME_API_KEY"
```

**`backend/src/onboarding_agent.py`:**
```python
GEMINI_API_KEY = "YOUR_ONBOARDING_API_KEY"
```

**`backend/src/policy_agent.py`:**
```python
...
The policy documents used by the backend live in `backend/src/data/policy_documents.json`.

The file is a simple JSON array of policy text entries:
...
Ensure MongoDB connection strings are configured in `backend/src/resume_agent.py` and `backend/src/onboarding_agent.py`.
...
**Form Fields:**
- `pdf_file` (required): Uploaded PDF resume file
- `job_role` (required): Target job role for scoring
- `policy_questions` (optional): JSON string of policy-related questions
